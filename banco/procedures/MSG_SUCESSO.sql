CREATE DEFINER=`dba`@`%` PROCEDURE `MSG_SUCESSO`(P_TEX_IDENTIFICADOR VARCHAR(30),
												 P_PARAM1            VARCHAR(100),
												 P_PARAM2            VARCHAR(100),
												 P_PARAM3            VARCHAR(100),
												 P_PARAM4            VARCHAR(100),
												 P_PARAM5            VARCHAR(100),
                                                 OUT P_OK	         CHAR(1),
...
DECLARE V_MSGSUCESSO         VARCHAR(2000);
 ...
 -- PEGA A MSG DE ERRO    
      ...
      IF V_MSGSUCESSO IS NULL THEN
         SET V_MSGSUCESSO = 'ERRO ! MENSAGEM DE SUCESSO NAO CADASTRADA PARA O PARAMETRO: ' || P_TEX_IDENTIFICADOR;
         SET P_OK 	      = 'N';
